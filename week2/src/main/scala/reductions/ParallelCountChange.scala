...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if ( threshold(money, coins) ) {
      countChange(money, coins) // sequential call
    } else {
      // parallel call
      if (money == 0) 1
      else if(money < 0 || coins.length <= 0) 0
      else {
        val result = common.parallel(parCountChange(money - coins.head, coins, threshold), parCountChange(money, coins.tail, threshold))
        result._1 + result._2
      }
    }
  }
  ...
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
    (money: Int, coins: List[Int]) => money * coins.length <= (startingMoney / 2) * allCoins.length
  }
}
