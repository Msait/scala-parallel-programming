...
package object barneshut {
  ...
  extends Quad {
    val (mass, massX, massY) = (
      bodies.foldLeft(0f)((acc,b) => acc + b.mass) : Float,
      bodies.foldLeft(0f)((acc,b) => acc + b.x * b.mass) / bodies.foldLeft(0f)((acc,b) => acc + b.mass): Float,
      bodies.foldLeft(0f)((acc,b) => acc + b.y * b.mass) / bodies.foldLeft(0f)((acc,b) => acc + b.mass) : Float)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = math.max( math.min(boundaries.maxX, b.x), boundaries.minX)
      val y = math.max( math.min(boundaries.maxY, b.y), boundaries.minY)
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      (0 until combinedMatrix.matrix.length).par.foreach( i =>
        combinedMatrix.matrix(i) = matrix(i).combine(that.matrix(i))
      )
      combinedMatrix
    }
